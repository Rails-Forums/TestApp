class BillingController < ApplicationController
  ...
  def subscribe
      if current_user.stripe_id.nil?
        redirect_to billing_path, :flash => {:error => 'Firstly you need to enter your card'}
        return
      end
      #if there is no card
   ...
   #we are creating a new subscription with the plan_id we took from our form
      ...
      subscription.save
      redirect_to root_path, notice: "You have successfully signed up and now enjoy are content."
  end
end
